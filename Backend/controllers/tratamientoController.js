...
exports.listarTratamientos = async (req, res) => {
  try {
    // Datos de respaldo para tratamientos/servicios
    const tratamientosRespaldo = [
      ...
      { id: 8, nombre: 'Puente Dental', descripción: 'Colocación de puente fijo', costo_estimado: 600000 }
    ];

    try {
      const result = await db.query('SELECT * FROM tratamientos LIMIT 10');
      if (result.rows.length > 0) {
        console.log(`✅ Tratamientos obtenidos desde BD: ${result.rows.length}`);
        res.json({ success: true, tratamientos: result.rows, source: 'database' });
        return;
      }
    } catch (dbError) {
      console.log('⚠️ Error de BD en tratamientos, usando respaldo:', dbError.message);
    }
    
    console.log(`✅ Usando tratamientos de respaldo: ${tratamientosRespaldo.length}`);
    res.json({ success: true, tratamientos: tratamientosRespaldo, source: 'fallback' });
  } catch (err) {
