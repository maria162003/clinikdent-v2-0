...
exports.crearFactura = async (req, res) => {
  try {
    ...
    const [ultimaFactura] = await db.query(
...
exports.eliminarFactura = async (req, res) => {
  try {
    ...
    await db.query('DELETE FROM detalles_factura WHERE factura_id = ?', [id]);
...
// ==========================================
// FUNCIÓN PARA GENERAR FACTURAS AUTOMÁTICAMENTE (NUEVA)
// ==========================================
exports.generarFacturaAutomatica = async (citaData) => {
  try {
    console.log('🧾 [AUTO-FACTURA] Generando factura automática para cita:', citaData.id);
    ...
    if (facturaExistente.rows.length > 0) {
      console.log('⚠️ [AUTO-FACTURA] Ya existe factura para esta cita');
      return { success: false, message: 'Factura ya existe' };
    }
    ...
    if (ultimaFactura.rows.length > 0) {
      const ultimoNumero = ultimaFactura.rows[0].numero_factura;
      const numeroSiguiente = parseInt(ultimoNumero.split('-')[1]) + 1;
      numeroFactura = `FAC-${numeroSiguiente.toString().padStart(6, '0')}`;
    }
    
    // Determinar precio del servicio
    let precio = 200000; // Precio base por defecto
    
    // Mapeo de tipos de cita a precios (igual que el sistema de comisiones)
    const preciosServicios = {
      ...
      'ortodoncia': 1500000,
      'implante': 3000000,
      'blanqueamiento': 400000,
      'endodoncia': 600000,
      'periodoncia': 300000,
      'cirugia': 1200000
    };
    
    // Buscar precio basado en el motivo de la cita
    if (citaData.motivo) {
      const motivoLower = citaData.motivo.toLowerCase();
      for (const [tipo, precioTipo] of Object.entries(preciosServicios)) {
        if (motivoLower.includes(tipo)) {
          precio = precioTipo;
          break;
        }
      }
    ...
    `, [
      ...
      citaData.odontologo_id,
      citaData.id,
      fechaEmision,
      fechaVencimiento.toISOString().split('T')[0],
      precio,
      'pendiente',
      `Factura generada automáticamente por cita completada - ${citaData.motivo}`
    ]);
    
    const facturaId = resultFactura.rows[0].id;
    const numeroGenerado = resultFactura.rows[0].numero_factura;
    
    console.log(`✅ [AUTO-FACTURA] Factura creada: ${numeroGenerado} por $${precio.toLocaleString()}`);
    ...
    console.error('❌ [AUTO-FACTURA] Error generando factura automática:', error);
