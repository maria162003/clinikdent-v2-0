...
const obtenerMetricasPrincipales = async (req, res) => {
    try {
        ...
        let queryTratamientos = `
            ...
            WHERE c.fecha BETWEEN ? AND ?
            AND t.estado IN ('en_progreso', 'planificado')
        `;
        let paramsTratamientos = [fecha_desde, fecha_hasta];
        
        if (sede_id) {
            queryTratamientos += ' AND c.sede_id = ?';
        ...
        const metricas = {
            totalIngresos: resultIngresos[0]?.total_ingresos || 0,
            totalCitas: resultCitas[0]?.total_citas || 0,
            totalPacientes: resultPacientes[0]?.total_pacientes || 0,
            totalTratamientos: resultTratamientos[0]?.total_tratamientos || 0,
            periodo: {
                desde: fecha_desde,
                hasta: fecha_hasta
            }
