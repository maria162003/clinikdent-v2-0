...
router.get('/public', async (req, res) => {
    try {
        ...
        console.error('Error en ruta public:', error);
        res.status(500).json({ 
            success: false, 
            error: error.message 
        });
...
// Ruta para probar solo equipos SIN autenticación
router.get('/equipos-test', async (req, res) => {
    try {
        const inventarioController = require('../controllers/inventarioController');
        await inventarioController.obtenerEquipos(req, res);
    } catch (error) {
        console.error('Error en equipos-test:', error);
        res.status(500).json({ 
            success: false, 
            error: error.message 
        });
...
// Ruta básica SIN autenticación para testing inicial - ORIGINAL
router.get('/public-old', async (req, res) => {
    try {
        const db = require('../config/db');
        const query = `
          SELECT 
...
// Nuevas rutas para CRUD individual de proveedores
...
// Nuevas rutas para CRUD individual de categorías
router.get('/categorias/:id', inventarioController.obtenerCategoriaPorId);
router.put('/categorias/:id', inventarioController.actualizarCategoria);
...
// Nuevas rutas para CRUD individual de equipos
router.get('/equipos/:id', inventarioController.obtenerEquipoPorId);
...
// Ruta duplicada eliminada - se usa la del controlador inventarioController.crearProveedor
...
// NOTA: Ruta /equipos ya está definida arriba en línea 86
