...
(function() {
    ...
    function configurarFechas() {
        ...
        const fechaFactura = document.getElementById('fechaFactura'); // ID corregido
        if (fechaFactura) fechaFactura.value = hoy;
        ...
        try {
            ...
            if (response.ok) {
                ...
                serviciosDisponibles = [
                    { id: 1, nombre: 'Consulta General', costo_base: 50000 },
                    { id: 2, nombre: 'Limpieza Dental', costo_base: 150000 },
                    { id: 3, nombre: 'Obturación Simple', costo_base: 200000 },
                    { id: 4, nombre: 'Endodoncia', costo_base: 450000 }
                ];
            }
        } catch (error) {
    ...
    async function cargarPacientes() {
        try {
            ...
            if (response.ok) {
                ...
                // Filtrar pacientes - puede ser por rol_id o nombre de rol
                pacientesDisponibles = usuarios.filter(u => 
                    u.rol === 'paciente' || 
                    u.rol_id === 3 || 
                    (u.role && u.role.toLowerCase() === 'paciente')
                );
                
                console.log('✅ Pacientes cargados:', pacientesDisponibles.length);
                console.log('📄 Estructura del primer paciente:', pacientesDisponibles[0]);
            } else {
    ...
    function abrirModalNuevaFactura() {
        console.log('🔄 Abriendo modal nueva factura...');
    ...
    function cargarPacientesFactura() {
        console.log('🔄 Cargando pacientes en modal factura...');
        const select = document.getElementById('pacienteFactura'); // ID corregido
        if (!select) {
        ...
        console.log('👥 Pacientes disponibles:', pacientesDisponibles.length);
        ...
        pacientesDisponibles.forEach(paciente => {
            ...
            option.textContent = `${paciente.nombre || 'N/A'} ${paciente.apellido || ''}`.trim();
            select.appendChild(option);
            console.log(`➕ Paciente agregado: ${option.textContent}`);
        });
    ...
    function cargarServiciosFactura() {
        console.log('🔄 Cargando servicios en modal factura...');
        const selects = document.querySelectorAll('.servicio-select');
        
        if (selects.length === 0) {
            console.error('❌ No se encontraron selects de servicios');
            return;
        }
        
        console.log('🛠️ Servicios disponibles:', serviciosDisponibles.length);
        ...
        selects.forEach((select, index) => {
            ...
            serviciosDisponibles.forEach(servicio => {
                ...
                option.textContent = servicio.nombre || servicio.descripcion || 'Servicio sin nombre';
                
                // Buscar el precio en diferentes campos posibles
                const precio = servicio.costo_base || servicio.costo_estimado || servicio.precio || servicio.costo || 0;
                option.dataset.precio = precio;
                
                select.appendChild(option);
                console.log(`➕ Servicio agregado: ${option.textContent} - $${precio}`);
            });
        });
        
        console.log('✅ Servicios cargados en selects');
    }

    function agregarServicioFactura() {
        console.log('➕ Agregando nuevo servicio a la factura...');
        const container = document.getElementById('serviciosFactura');
        if (!container) return;

        const newRow = document.createElement('div');
        newRow.className = 'row align-items-end mb-2 servicio-item';
        newRow.innerHTML = `
            <div class="col-md-5">
            ...
            <div class="col-md-2">
                <button type="button" class="btn btn-outline-danger w-100" 
                        onclick="eliminarServicioFactura(this)">
        ...
        // Cargar servicios en el nuevo select
    ...
    function eliminarServicioFactura(button) {
        const row = button.closest('.servicio-item');
        if (row) {
            row.remove();
            calcularTotalFactura();
    ...
    async function guardarFactura() {
        ...
        const paciente = document.getElementById('pacienteFactura').value;
        ...
        // Recopilar servicios
        const servicios = [];
        document.querySelectorAll('.servicio-select').forEach((select, index) => {
    ...
    function calcularTotalFactura() {
        ...
        // Si no hay servicios con clase .servicio-item, usar los inputs directos
        if (subtotal === 0) {
        ...
        console.log(`💰 Cálculo: Subtotal=${subtotal}, Total=${total}`);
    }
        ...
        if (serviciosDisponibles.length) {
            tbody.innerHTML = serviciosDisponibles.map(servicio => `
                <tr>
                    ...
                    <td>${formatter.format(servicio.costo_base || servicio.costo_estimado || 0)}</td>
                    <td>
    ...
    window.agregarServicioFactura = agregarServicioFactura;
    window.eliminarServicioFactura = eliminarServicioFactura;
