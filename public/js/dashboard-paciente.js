...
class DashboardPaciente {
    ...
    calculateDashboardStats() {
        ...
        const citasFuturas = this.citas.filter(cita => {
            try {
                // Mejor manejo de la fecha - usar ISO format o construcción manual
                const fechaStr = cita.fecha.split('T')[0]; // YYYY-MM-DD
                const [year, month, day] = fechaStr.split('-');
                const [hour, minute] = cita.hora.split(':');
                
                // Crear fecha con constructor específico (mes -1 porque Date usa 0-11)
    ...
    renderCitasTable() {
        ...
        this.citas.forEach(cita => {
            ...
            // Formatear nombre del odontólogo
            const odontologoNombre = cita.odontologo_nombre && cita.odontologo_apellido 
                ? `Dr. ${cita.odontologo_nombre} ${cita.odontologo_apellido}`
                : 'Dr. Por asignar';
    ...
    async guardarCita() {
        ...
        try {
            ...
            if (response.ok) {
                // Mostrar mensaje personalizado basado en si fue confirmada automáticamente
                let mensaje = '✅ Cita agendada exitosamente.';
                if (result.confirmadaAutomaticamente) {
                    mensaje += ' Su cita ha sido confirmada automáticamente y ya aparece en el calendario del odontólogo asignado.';
                } else {
                    mensaje += ' Su cita está pendiente de confirmación debido al horario seleccionado. Recibirá una notificación cuando sea confirmada.';
                }
                
                this.showAlert(mensaje, 'success');
                
                // Mostrar información adicional del odontólogo asignado
                if (result.odontologoAsignado) {
                    setTimeout(() => {
                        this.showAlert(`🦷 Odontólogo asignado: Dr. ${result.odontologoAsignado.nombre} ${result.odontologoAsignado.apellido}`, 'info');
                    }, 2000);
                }
    ...
    async loadPagos() {
        try {
            ...
            if (!responseFacturas) {
                console.log('⚠️ No se pudieron cargar las transacciones');
                this.displayFacturasPendientes(true);
                this.displayHistorialPagos(true);
                return;
            }
        ...
        if (!this.facturasPendientes || this.facturasPendientes.length === 0) {
            container.innerHTML = `
                <div class="text-center py-4 text-muted">
                    ...
                    <small>Todos tus pagos están al día</small>
                </div>
            `;
            return;
        }
        ...
        if (!this.facturasPagadas || this.facturasPagadas.length === 0) {
            container.innerHTML = `
                <div class="text-center py-4 text-muted">
                    ...
                    <small>Aquí aparecerán tus pagos completados</small>
                </div>
            `;
            return;
        }
    ...
    // Función para procesar pagos con MercadoPago
    async pagarConMercadoPago(facturaId, monto, descripcion) {
        ...
        try {
            ...
            // Mostrar loading - buscar el botón de pago
            ...
            if (data.success && data.init_point) {
                // Mostrar modal con información del pago
                            ...
                            <div class="col-md-6">
                                <div class="card h-100">
                                    <div class="card-body text-center">
                                        ...
                                        <p class="text-muted small">Escanea el código QR</p>
                        ...
                        <div class="mt-3">
                            <div class="alert alert-info">
                                ...
                                <ul class="mb-0 mt-2">
                                    ...
                                    <li>Una vez completado el pago, se actualizará automáticamente aquí</li>
                                </ul>
                            </div>
                        </div>
                    </div>
    ...
    configurarEvaluacionForm() {
        ...
        estrellas.forEach((estrella, index) => {
            estrella.addEventListener('mouseenter', () => {
                this.resaltarEstrellas(estrella.dataset.campo, index % 5 + 1);
            });
            
            estrella.addEventListener('click', () => {
                this.seleccionarEstrellas(estrella.dataset.campo, index % 5 + 1);
            });
        });
        ...
        // Configurar envío del formulario mediante el botón
    ...
    // Configurar formulario de pago rápido
        ...
        if (tipoServicioSelect && precioInput && descripcionInput) {
            ...
            tipoServicioSelect.addEventListener('change', function() {
                ...
                if (precio && precio !== '0') {
                    ...
                    descripcionInput.value = servicio + ' - Clínica Dental Clinikdent';
                } else if (this.value === 'otro') {
                    precioInput.value = '';
                    descripcionInput.value = 'Servicio personalizado - Clínica Dental Clinikdent';
                    precioInput.focus();
                }
            });
        }
    ...
    async procesarPagoRapido() {
        try {
            ...
            if (!userId) {
                this.showAlert('Error: No se pudo obtener la información del usuario', 'danger');
                return;
            }

            // Determinar título del servicio
            const tipoServicioSelect = document.getElementById('tipoServicio');
            ...
            console.log('💳 Procesando pago rápido:', { titulo, precio, descripcion });
    ...
    // Método para cancelar factura pendiente
