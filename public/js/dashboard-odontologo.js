...
class DashboardOdontologo {
    ...
    async loadUserInfo() {
        try {
            ...
            const response = await this.authFetch(`/api/usuarios/${userId}/perfil`);
            if (!response.ok) {
    ...
    async calculateDashboardStats(userId) {
        try {
            ...
            const response = await this.authFetch(`/api/citas/agenda/odontologo?id_odontologo=${userId}`);
            if (!response.ok) {
    ...
    async loadAgendaHoy() {
        try {
            ...
            container.innerHTML = citasHoy.map(cita => {
                const hora = new Date(`${cita.fecha}T${cita.hora}`).toLocaleTimeString('es-ES', {
                    hour: '2-digit',
                    minute: '2-digit'
                });
                ...
                return `
                    <div class="timeline-item">
                        ...
                        <div class="timeline-content">
                            <div class="timeline-title">Paciente ID: ${cita.paciente_id}</div>
            ...
            container.innerHTML = proximasCitas.map(cita => {
                ...
                return `
                    <div class="list-group-item d-flex justify-content-between align-items-center">
                        <div>
                            <h6 class="mb-1">Paciente ID: ${cita.paciente_id}</h6>
                ...
                citaCard.innerHTML = `
                    <div class="card-body">
                        <div class="row align-items-center">
                            ...
                            <div class="col">
                                <div class="agenda-paciente">Paciente #${cita.paciente_id}</div>
                                ...
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <div class="btn-group btn-group-agenda" role="group">
                                        ...
                                        ${cita.estado === 'programada' ? `
                                            <button class="btn btn-outline-success btn-sm" onclick="dashboardOdontologo.confirmarCita(${cita.id})" title="Confirmar cita">
    ...
    async loadPacientes() {
        try {
            ...
            const response = await this.authFetch(`/api/usuarios/${userId}/pacientes`);
            if (!response.ok) {
            ...
            console.log('📊 Array de pacientes procesado:', this.pacientes);
            this.renderPacientesTable();
            
        } catch (err) {
            console.error('❌ Error al cargar pacientes:', err);
            const tbody = document.querySelector('#pacientesTable tbody');
            tbody.innerHTML = '<tr><td colspan="6" class="text-center text-danger">Error al cargar pacientes</td></tr>';
        }
    }
    ...
    updatePacientesStats() {
        ...
        if (document.querySelector('#totalPacientes')) {
            document.querySelector('#totalPacientes').textContent = totalPacientes;
        }
        ...
        console.log(`📊 Stats pacientes: Total: ${totalPacientes}, Activos: ${pacientesActivos}, Inactivos: ${pacientesInactivos}`);
    }
    ...
    async verHistorialPaciente(pacienteId) {
        ...
        try {
            ...
            const [citasResponse, historialesResponse] = await Promise.all([
                this.authFetch(`/api/citas/${pacienteId}`),
                this.authFetch(`/api/historial/paciente/${pacienteId}`)
            ]);
    ...
    async verCitasPaciente(pacienteId) {
        ...
        try {
            ...
            const response = await this.authFetch(`/api/citas/${pacienteId}`);
            if (!response.ok) {
    ...
    async deletePaciente(id) {
        ...
        try {
            await this.authFetch(`./api/pacientes/${id}`, { 
                method: 'DELETE',
    ...
    async loadHistoriales() {
        try {
            ...
            const pacientesResponse = await this.authFetch(`./api/usuarios/${userId}/pacientes`);
            ...
            for (const paciente of pacientes) {
                try {
                    ...
                    const historialResponse = await this.authFetch(`/api/historial/paciente/${paciente.id}`);
    ...
    async loadHistorialData(id) {
        try {
            const response = await this.authFetch(`/api/historial/${id}`);
    ...
    async saveHistorial() {
        ...
        try {
            ...
            if (this.currentHistorialId) {
                ...
                response = await this.authFetch(`/api/historial/${this.currentHistorialId}`, {
                    method: 'PUT',
                    body: JSON.stringify(data)
                });
            } else {
                // Create new historial
                response = await this.authFetch('/api/historial', {
                    method: 'POST',
                    body: JSON.stringify(data)
                });
            }
        ...
        const notifications = citas.map(cita => {
            ...
            return `
                <div class="dropdown-item">
                    ...
                    <p class="mb-1">Paciente ID: ${cita.paciente_id}</p>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                            ${cita.estado === 'programada' ? `
                                <button type="button" class="btn btn-success" onclick="dashboardOdontologo.confirmarCita(${cita.id}); bootstrap.Modal.getInstance(document.getElementById('modalDetallesCita')).hide();">
                                    <i class="bi bi-check-square me-1"></i>Confirmar
                                </button>
                            ` : ''}
                            ${cita.estado !== 'completada' && cita.estado !== 'cancelada' ? `
                                <button type="button" class="btn btn-primary" onclick="dashboardOdontologo.completarCita(${cita.id}); bootstrap.Modal.getInstance(document.getElementById('modalDetallesCita')).hide();">
    ...
    eliminarCita(citaId) {
        ...
        if (confirm(`⚠️ ELIMINAR CITA PERMANENTEMENTE\n\nFecha: ${fecha} a las ${hora}\nPaciente: #${cita.paciente_id}\nMotivo: ${cita.motivo || 'Consulta general'}\n\n¿Está seguro de que desea ELIMINAR definitivamente esta cita?\n\n⚠️ ESTA ACCIÓN NO SE PUEDE DESHACER`)) {
            ...
            if (confirm('CONFIRMACIÓN FINAL:\n\n¿Realmente desea ELIMINAR esta cita de forma PERMANENTE?\n\nEsta acción es irreversible.')) {
                this.authFetch(`./api/citas/${citaId}/eliminar`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        ...this.getAuthHeaders()
    ...
    actualizarEstadoCita(citaId, nuevoEstado, mensajeExito, notas = null) {
        ...
        this.authFetch(`./api/citas/${citaId}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                ...this.getAuthHeaders()
    ...
    async loadMisCitas() {
        try {
            ...
            const response = await this.authFetch(`/api/citas/odontologo/${userId}`);
                            ...
                            <tr class="${this.getRowClass(cita.estado)}">
                                ...
                                <td>
                                    <div class="btn-group-vertical btn-group-sm">
                                        <button class="btn btn-outline-primary btn-sm" onclick="dashboardOdontologo.verDetallesCita(${cita.id})" title="Ver detalles">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        ${cita.estado !== 'completada' && cita.estado !== 'cancelada' ? `
                                            <button class="btn btn-outline-success btn-sm" onclick="dashboardOdontologo.completarCita(${cita.id})" title="Marcar como completada">
                                                <i class="fas fa-check"></i>
                                            </button>
                                        ` : ''}
                                        ${cita.estado === 'completada' ? `
                                            <button class="btn btn-outline-info btn-sm" onclick="dashboardOdontologo.verHistorialPaciente(${cita.paciente_id})" title="Ver historial del paciente">
    ...
    async loadResumenFinanciero() {
        try {
            ...
            if (data.success) {
                ...
                this.displayGraficosFinancieros(data.ingresos_mensuales, data.top_tratamientos);
            }
    ...
    displayGraficosFinancieros(ingresosMensuales, topTratamientos) {
        // Gráfico de ingresos mensuales
        this.createChartIngresosMensuales(ingresosMensuales);
        
        // Gráfico de top tratamientos
        this.createChartTopTratamientos(topTratamientos);
    }
    ...
    createChartTopTratamientos(data) {
        const ctx = document.getElementById('chartTopTratamientos');
        if (!ctx) return;
    ...
    verDetalleFactura(facturaId) {
        ...
        // Implementar modal con detalle de factura
        alert(`Ver detalle de factura ID: ${facturaId}\n(Funcionalidad en desarrollo)`);
    }
