...
function showNotification(message, type = 'success', duration = 5000) {
    // Usar el sistema de alertas modernas si está disponible
    if (window.showNotificationMejorada) {
        return window.showNotificationMejorada(message, type, duration);
    }
    
    // Usar showCustomAlert como fallback
    if (window.showCustomAlert) {
        const alertTypes = {
            'success': { title: '✅ Éxito', type: 'success' },
            'error': { title: '❌ Error', type: 'error' }, 
            'danger': { title: '⚠️ Peligro', type: 'error' },
            'warning': { title: '⚠️ Advertencia', type: 'warning' },
            'info': { title: 'ℹ️ Información', type: 'info' }
        };
    ...
    // Mapear tipos de iconos (fallback final)
    const iconMap = {
